use std::{io::{Error, Write}, net::{TcpListener, TcpStream}, thread, time::{Duration, Instant}};

use raspi_oled::github::get_new_notifications;

fn main() {
    let pat = Box::leak(Box::new(std::env::var("GITHUB_PAT").expect("no env var GITHUB_PAT set")));
